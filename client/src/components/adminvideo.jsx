...
export default function AdminVideo() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await axiosClient.delete(`/video/delete/${selectedToDelete._id}`);
    ...
    <div className="min-h-screen p-6 bg-gradient-to-b from-slate-900 to-slate-950 text-slate-100">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-semibold">Manipulate Videos</h1>
            <p className="text-sm text-slate-400">Create, Delete and Update Videos</p>
          </div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="inline-flex items-center gap-2">
                       <NavLink to={`/admin/upload/${problem._id}`}
                        className={"px-3 py-1 rounded-md bg-success hover:bg-success/80 text-success-content text-sm font-medium shadow-md shadow-success/40"}
                        >
                        Upload
                       </NavLink>
                      </div>
